...
export function MailDisplay({ mail }: MailDisplayProps) {
  ...
  useEffect(() => {
    const updateView = async (mailId: string) => {
      await updateTicket(mailId, {
        viewed: true,
      });
        ...
        <div className="flex flex-1 flex-col">
          <div className="flex items-start p-4">
            <div className="flex items-start gap-4 text-sm">
              <Avatar>
                <AvatarImage
                  alt={mail.user.xAccount?.name || "Unknown"}
                  src={mail.user.xAccount?.profile_image_url}
                />
                <AvatarFallback>
                  {mail.user.xAccount?.name
                    .split(" ")
                    .map((chunk) => chunk[0])
                    .join("")}
                </AvatarFallback>
              ...
              <div className="grid gap-1">
                {mail.user.xAccount?.name && (
                  <div className="font-semibold">{mail.user.xAccount.name}</div>
                )}
                <div className="line-clamp-1 text-xs">
                  {formatAddress(mail.user.address)}
                </div>
                <div className="line-clamp-1 text-xs">
