...
const useGetWalletWorth = () => {
    ...
    const {accountToken: egldToken} = useGetAccountToken(selectedNetwork.tokensID.egld)
    ...
    useEffect(() => {
      if (egldPrice ) {
        ...
        if (tokensData.length > 0) {
            ...
            if (token.price) {
              total += new BigNumber(getRealBalance(token.balance, token.decimals)).multipliedBy(token.price).toNumber()
            }
          });
        }
